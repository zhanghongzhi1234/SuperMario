#ifndef __UTILITY_TOOL
#define __UTILITY_TOOL
...
//Vertical line collides with rectangle
#define VLINE_IN_RECT(x,y,h,x1,y1,w1,h1) ( (y)+(h)>(y1) && (y)<(y1)+(h1) \
                                    && (x)>(x1) && (x)<(x1)+(w1) )

//Horizontal line collides with rectangle
...
int FGetLineJumpCom(char *pchar, FILE *fp)
{
    ...
    //Filter the preceding illegal characters
